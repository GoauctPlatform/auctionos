...
print("--- TESTING DATA MAPPING FROM REMOTE DB ---")
try:
    ...
    with engine.connect() as conn:
        ...
        params = {
            "skip": 0,
            "limit": 100,  # Try 100 items to find potential data issues
            "uid": 40,
            "cid": None
        }
        ...
        for i, r in enumerate(result):
            try:
                ...
                print(f"Row {i} failed to map! Data: {r}")
                raise row_err
                
        print(f"Mapped {len(items)} items successfully!")
