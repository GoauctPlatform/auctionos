...
print("--- TESTING PYDANTIC VALIDATION ON DB ROWS ---")
try:
    ...
    with engine.connect() as conn:
        ...
        for i, r in enumerate(result):
            ...
            try:
                ...
                obj = PropertyDashboardSchema(**item_dict)
                validated_count += 1
            except Exception as val_err:
                print(f"Pydantic validation FAILED for row {i} (parcel_id: {r[0]}):")
                print(val_err)
                sys.exit(1)
                
        print(f"Successfully validated {validated_count} rows against PropertyDashboardSchema!")
